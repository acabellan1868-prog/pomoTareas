...
## Cambios — 11/03/2026 (v11)

### Temporizador resistente al throttling del navegador

**Problema corregido**
- Chrome (y otros navegadores basados en Chromium) limitan la frecuencia de `setInterval` cuando la pestaña pierde el foco o la ventana se minimiza. Esto hacía que el temporizador contase de menos: el reloj se ralentizaba o parecía pausarse en segundo plano.

**Solución implementada: cálculo por timestamps**
- El objeto `timer` incorpora dos nuevos campos:
  - `timestampInicio` — marca de tiempo exacta (ms desde epoch) en que se inicia o reanuda la fase.
  - `secondsLeftAlIniciar` — segundos que quedaban en ese momento.
- La función `tick()` ya no decrementa un contador. En su lugar calcula `Math.floor((Date.now() - timestampInicio) / 1000)` y lo resta de `secondsLeftAlIniciar`. Así el tiempo mostrado es siempre el real, independientemente de la frecuencia con la que el navegador ejecute el intervalo.
- El intervalo de `setInterval` se redujo de 1000 ms a 500 ms para que la pantalla se actualice con más fluidez al volver al foco (no afecta a la precisión del conteo).
- Al pausar, `secondsLeft` ya tiene el valor correcto, y al reanudar se vuelven a guardar los nuevos valores de referencia en `startTimer()`.
- El comportamiento externo (iniciar, pausar, continuar, reiniciar, saltar fase) es idéntico al anterior.
